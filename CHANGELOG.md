...
Suikan（随看）的所有可记录改动。**V1.0 之前**（2026-08-22 及之前）项目名为 bilidown，commit 历史保留。按"用户能感知到的影响"维度写，不按 commit 维度。

## V1.0（2026-08-23）：项目重命名 + 定位升级 + 双版权

**这是 Suikan/随看的第一个正式 release**。此前的所有 commit 都属于 bilidown V0.x 早期版本，**功能上完全连续**——本次变更**只动名字、定位文案、license 对齐、i18n**，**不动任何业务代码**。

### 1. 项目重命名：`bilidown` → `Suikan 随看`

**为什么改名**：

- `bilidown` 暗示"B 站下载工具"——但本项目**核心价值**是"边看边学、笔记沉淀"，**不是下载**。
- B 站是当前主战场，但项目方向是**通用化"长视频 → 学习资料"**——名字里不能带 `B 站` / `bilibili` / 字母 `B`。
- "随看"中英文 slogan 完美对仗：
  - 中文：**随时看，随手记。**（"看"和"记"两件事随时发生）
  - 英文：**Watch less. Learn more.**（"少看多学"——效率导向）

**做了哪些重命名**：

- 扩展名 `manifest.json` 的 `name` 从 `bilidown` → `Suikan 随看`（i18n message `appName`）
- `package.json` 的 `name` 从 `bilidown` → `suikan`（npm 包名干净，未被占用）
- 仓库名 GitHub `WezheShang/bilidown` → `WezheShang/suikan`（用户**手动**在 github.com Settings 里改名；本机 `git remote set-url` 同步）
- README.md / NOTICE / CHANGELOG.md / package.json / manifest.json 全文 bilidown 引用替换为 Suikan/随看
- 保留所有 commit history——任何人都能 `git log` 看到从 bilidown 演进到 Suikan 的完整轨迹

### 2. 定位升级：B 站专用 → 通用学习视频助手

虽然当前 B 站仍是主战场（content.js 第一优先级适配），代码结构已经按"按平台适配"组织：

- `content.js` 检测当前 URL 走对应平台分支
- ASR / AI 概览 / 笔记 / 导出逻辑**完全共享**
- **新增平台 = 加一个 content script 适配 + 一个 platform-specific fetcher**，核心 pipeline 复用

README 的"当前支持的平台"表现在写成 B 站 ✅ / YouTube 🚧 / 其他 ⏳。

### 3. 双版权 NOTICE 文件

新增 `NOTICE` 文件（原 `LICENSE` 不动），按 MIT 推荐的方式声明：

- **Original work**: youtube-digest, Zara Zhang, MIT, (c) 2026
- **Modifications copyright (c) 2026 WezheShang**（你自己的修改版权）
- 详细列出本 fork 做了什么修改
- 保留哪些不变（核心产品理念、MIT License 条款、原始版权行）

`package.json` 同步修正：

- `author`: `""` → `{ name: "WezheShang", url: "https://github.com/WezheShang" }`
- `license`: `"ISC"` → `"MIT"`（与 LICENSE 文件对齐——**之前写错了**）
- `repository.url`: **去掉了内嵌的 GitHub PAT**（之前 `git+https://ghp_...@github.com/...` 把 token 写进 package.json，npm publish 会泄漏）
...
- `appShortName`: `Suikan`
- `appTagline`: `随时看，随手记。`
- `appDescription`: 完整长描述

`manifest.json` 同步：

- 加 `default_locale: "zh_CN"`
- `name` → `__MSG_appName__`
- `description` → `__MSG_appDescription__`
- 加 `short_name: __MSG_appShortName__`（Chrome Web Store 空间不足时用）

**只做中文 i18n**（英文环境 fallback 到 manifest name 字段；后续按需加 `_locales/en/`）。

### 5. README 全面重写

- 顶部标题：bilidown → Suikan 随看
- 加 V1.0 公告 callout：说明重命名 + 定位升级
- 第一段：定位从"B 站 AI 总结" → "通用学习视频助手"
- 新增"项目历史"表（V0.x bilidown → V1.0 Suikan）
- 所有功能描述、技术栈、安装步骤保留并按 Suikan 上下文重写

### 6. 扩展 ID 稳定性保留

`manifest.json` 第 5 行的固定 `key`（公钥）**不变**——升级到 V1.0 时**扩展 ID 不变**，用户已有的 `chrome.storage.local` API Key 不会丢。

### 用户操作

1. 升级时：直接覆盖到原文件夹 → `chrome://extensions` 重新加载。**不要**先删除旧扩展（删除会清 API Key）。
2. 第一次从 V0.x 切到 V1.0：因为扩展 ID 可能变化一次（key 字段从无到有），需要重新填写一次 API Key。
3. GitHub 仓库名需要**手动**在 github.com 网页改：`WezheShang/bilidown` → `WezheShang/suikan`。本机 `git remote set-url origin https://github.com/WezheShang/suikan.git` 同步。

---

## Unreleased

> 自 V1.0 起，Unreleased 区域记录**尚未 release 的**变更。V1.0 之前的变更记录见下面 0.x 章节。
