...
bilidown 会直接显示在哔哩哔哩视频页面中。配置阿里云百炼 API Key 后，默认使用 Fun-ASR 识别当前视频音轨；未配置百炼 Key 时，才通过 B 站 WBI 播放器接口读取原生字幕。获得文字后，再由 minimax 生成中文概览、章节时间线和关键观点。
...
| 服务 | 是否必需 | 用途 |
| --- | --- | --- |
| minimax API Key | 必需 | 概览、章节、观点、翻译和笔记整理 |
...
只要扩展 ID 没有变化，`chrome.storage.local` 中的 minimax 和百炼 API Key 就会继续保留。通过 Chrome Web Store 或 Microsoft Edge 加载项商店正常升级时，扩展 ID 同样保持不变，一般不需要重新填写 Key。
...
- ASR、minimax 的免费额度、价格和可用性由对应平台决定。
