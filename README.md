# bililearn 📺

> **把 B 站长视频，变成可以阅读、跳转、总结和保存的中文学习资料。**
> 一个适用于 Chrome、Microsoft Edge 及其他 Chromium 浏览器的 B 站视频 AI 总结扩展。

bililearn 会直接显示在哔哩哔哩视频页面中。默认情况下，扩展会通过 B 站播放器接口读取视频已有的官方字幕（人工或 AI 翻译）。对于**没有官方字幕**的视频，用户可以在设置里启用本地 Whisper 转写，由本机 `faster-whisper` 完成识别。获得文字后，再由 LLM（默认 minimax）生成中文概览、章节时间线和关键观点。

它不是简单地把整段字幕丢给 AI，而是围绕 B 站真实使用场景处理了几个关键问题：**视频切换、分 P / 合集识别、时间戳跳转、播放位置跟随、全屏笔记、中文章节和多格式导出。**


> [!IMPORTANT]
> 本项目基于 [zarazhangrui/youtube-digest](https://github.com/zarazhangrui/youtube-digest) 二次开发，针对哔哩哔哩网页结构和国内服务重新适配，不是哔哩哔哩官方产品。

---

## ✨ 核心功能

### 1. B 站原生页面集成

扩展会在视频操作栏中加入 **AI 总结** 按钮：

- 固定跟随在“转发”按钮右侧
- 页面放大、缩小后仍跟随操作栏布局
- 点击后打开浏览器侧边栏
- 不需要复制视频链接到其他网站
- 支持 B 站单页应用的视频切换

侧边栏包含：

```text
字幕
概览
笔记
```

---

### 2. 当前视频识别

B 站可能在不刷新浏览器的情况下切换 BV、分 P 或合集分集。

bililearn 会根据当前页面的：

```text
BV 号
分 P 参数
合集 / 分集标识
当前活动视频链接
```

创建独立状态和缓存，避免打开第二个视频后仍显示第一个视频的字幕或总结。

---

### 3. 默认只读 B 站官方字幕，Whisper 按需启用

当前版本的文字来源策略是 **B 站官方字幕优先，Whisper 按需启用**：

```text
打开任意 B 站视频
↓
扩展自动通过 /x/player/wbi/v2 读取官方字幕
（含中文 / 英文 / AI 翻译字幕，按系统语言优先选择）
↓
侧边栏立即可读
```

只有在以下两种情况，扩展才会**真的**去转录音轨：

1. 该视频没有任何官方字幕（老视频、UP 主自己关的、地区限制）
2. 用户在扩展设置里手动把「语音识别」切到「本地 Whisper」并完成本机环境检查

这样可以：

- 不打扰只想看官方字幕的用户（默认零后台任务）
- 不把任何音轨上传到第三方服务（除非用户自己接入外部 ASR）
- 让本机 CPU / 磁盘只在用户主动需要时跑起来

扩展永远不会在打开页面时自动预加载 ASR。只有点击 AI 总结后才会触发，从而避免：

- 不必要的流量消耗
- 页面打开后的后台任务
- 用户只是浏览视频时的等待

---

### 4. 中文 AI 概览

minimax 会根据当前视频字幕或 ASR 结果生成：

- 视频内容概览
- 简体中文章节时间线
- 每章内容说明
- 关键观点
- 值得记录的原话
- 可继续整理的学习笔记

章节标题和说明会明确要求使用简体中文，不会因为字幕中混有英文术语就生成整套英文目录。

---

### 5. 播放位置联动

点击字幕或概览中的时间戳，播放器会跳转到对应位置：

```text
00:27 → 跳转到视频第 27 秒
04:18 → 跳转到视频第 4 分 18 秒
```

字幕列表也可以跟随当前播放进度，方便边看视频边阅读内容。

---

### 6. 普通页面与全屏笔记

除了侧边栏中的笔记页面，扩展也适配了播放器全屏场景。

可以在观看过程中记录：

- 当前时间戳
- 临时想法
- 视频重点
- 待办事项
- 需要回看的片段

笔记会绑定当前 BV 和分集，不会混入其他视频。

---

### 7. 多格式导出

当前支持：

```text
Markdown
HTML
复制到飞书
```

适合继续整理到飞书文档、飞书知识库、Obsidian、Notion、Typora、个人博客或本地 HTML 文件。

---

## 🌐 浏览器兼容性

| 浏览器 | 建议版本 | 支持情况 |
| --- | ---: | --- |
| Google Chrome | 116+ | ✅ 主要支持 |
| Microsoft Edge | 116+ | ✅ 主要支持 |
| 其他 Chromium 浏览器 | 116+ | ⚠️ 取决于 Side Panel API 支持情况 |
| Firefox | — | ❌ 当前不支持 |
| Safari | — | ❌ 当前不支持 |

Chrome 和 Microsoft Edge 使用同一个扩展目录和安装包，**不需要单独下载 Edge 版本**。

其他 Chromium 浏览器需要支持：

```text
Manifest V3
Side Panel API
Declarative Net Request
chrome.storage.local
```

---

## 🚀 快速安装

### 方法一：加载发布包

1. 下载 `bililearn-main.zip`。
2. 将 ZIP 完整解压到固定文件夹。
3. 不要在加载扩展后删除或移动该文件夹。

#### Chrome

1. 打开 `chrome://extensions/`。
2. 开启右上角的 **开发者模式**。
3. 点击 **加载已解压的扩展程序**。
4. 选择刚才解压的文件夹。

#### Microsoft Edge

1. 打开 `edge://extensions/`。
2. 开启左侧的 **开发人员模式**。
3. 点击 **加载解压缩的扩展**。
4. 选择刚才解压的文件夹。

安装完成后，刷新已经打开的 B 站视频页面。

---

### 方法二：从源码加载

```bash
git clone <你的仓库地址>
cd bililearn
```

然后在 Chrome 或 Edge 的扩展管理页面中直接选择项目根目录。

扩展运行不依赖 npm；npm 只用于测试和生成发布包。

---

## 🔑 API 配置

| 服务 | 是否必需 | 用途 |
| --- | --- | --- |
| LLM API Key（minimax / DeepSeek / GLM 等） | 必需 | 概览、章节、观点、翻译和笔记整理 |
| Whisper 本机环境 | 可选 | 只在没有官方字幕且你主动启用时使用 |

点击扩展的 **设置**，填写对应 API Key 后保存即可。

密钥只保存在当前浏览器的扩展本地存储中。重新安装扩展或清除扩展数据后，需要重新配置。

### 🎙️ 按需启用：本地 Whisper 离线 ASR

> **大多数用户可以跳过这一节。** 默认情况下扩展只会读 B 站官方字幕。本地 Whisper 是在视频没有官方字幕时、你想让本机帮你转录的备选方案 —— 适合不想把音轨发给第三方、不在意本机 CPU/磁盘占用的用户。

本地 Whisper 让你**完全离线**完成 ASR：

- 音轨**不**上传任何第三方服务，留在本机
- 模型可选用更准的 `medium` / `large-v3`（前提是内存/CPU 够）
- 中文识别走 `faster-whisper` + `zhconv`（繁→简）
- 启用流程：扩展设置 → 语音识别 → 选「本地 Whisper」→ 点「检查系统」按提示补齐本机依赖

#### 什么时候选本地 Whisper？

| 场景 | 推荐方案 |
| --- | --- |
| 视频有 B 站官方字幕 | 默认 WBI 字幕（**不需要 ASR**） |
| 视频无字幕 + 不想花钱 + 不在意本机 CPU | **本地 Whisper** |
| 视频无字幕 + 不想自己装环境 | 当前版本不覆盖（之前的云端 ASR 方案已下线，需要换服务的用户可改写 `sidepanel.js` 接入） |
| 隐私敏感（音轨不能出本机） | **本地 Whisper** |

#### 系统依赖

- **Python 3.10+**（任意安装方式：python.org、miniconda、anaconda 都行）
- **`faster-whisper`** Python 包
- **`zhconv`** Python 包（繁→简转换）
- **足够的磁盘空间**：tiny 模型 ~75 MB，base ~150 MB，small ~500 MB，medium ~1.5 GB，large-v3 ~3 GB
- **7860 端口空闲**（本机 server 用）

#### 安装步骤

1. **装 Python**（如果还没装）：https://www.python.org/downloads/ ，勾选 "Add Python to PATH"
2. **装依赖**：

   ```bash
   pip install faster-whisper zhconv
   ```

3. **启 server**：

   - 双击 `start_whisper_server.bat`（开窗口的版本）
   - 或双击 `start_whisper_server_silent.vbs`（无窗口）
   - 第一次跑会下载默认模型（`base` 约 150 MB）

4. **在扩展设置里选 "本地 Whisper"**：
   - "测试连接" → 应该看到 `{"ok": true, ...}` 表示通了

#### 常见坑

- **bat 里写死了 Python 路径？** v2.0+ 已修复——bat 会按以下顺序找 Python：`BILILEARN_PYTHON` 环境变量（改名前的旧名 `BILIDOWN_PYTHON` 仍然有效）→ `where python` → 常见安装位置。找不到会**给出明确报错**告诉你装哪
- **启了 server 但连不上？** 浏览器扩展需要 `http://127.0.0.1:7860` 权限（manifest 已声明），防火墙可能会拦——第一次测试连接 Windows 会弹窗问是否放行
- **server 不会自启？** v2.0+ 推荐做法见 [§ 服务器自启](#-服务器自启-windows)，创建开机任务计划
- **音轨下载报 403？** 跟 ASR 路径无关，B 站临时鉴权过期，刷新页面重试

#### 推荐模型

| 模型 | 磁盘 | 速度 | 准确度 |
| --- | ---: | --- | --- |
| `tiny` | 75 MB | ⚡⚡⚡⚡⚡ | ⭐⭐ |
| `base` | 150 MB | ⚡⚡⚡⚡ | ⭐⭐⭐ |
| `small` | 500 MB | ⚡⚡⚡ | ⭐⭐⭐⭐ |
| `medium` | 1.5 GB | ⚡⚡ | ⭐⭐⭐⭐⭐ |
| `large-v3` | 3 GB | ⚡ | ⭐⭐⭐⭐⭐+ |
| `turbo` | 1.5 GB | ⚡⚡⚡ | ⭐⭐⭐⭐⭐ |

中文推荐 `small` 或更高。CPU-only 机器用 `medium` 就到顶了，再大会卡。

### 更新时保留 API Key

**不要先删除旧扩展。** Chrome 官方说明，扩展被移除时会清除它的 `chrome.storage.local`；即使稍后装回相同扩展 ID，已经删除的本地 Key 也不会自动回来。

也不要每次把新版本解压到不同文件夹后删除重装。固定身份公钥可以稳定扩展 ID，但不能阻止浏览器在“卸载”操作时主动清除本地存储。

开发者模式下建议使用固定目录更新：

```text
保留原扩展文件夹
      ↓
把新版本文件覆盖到原文件夹
      ↓
打开 chrome://extensions/ 或 edge://extensions/
      ↓
点击该扩展的“重新加载”
```

只要扩展 ID 没有变化，`chrome.storage.local` 中的 LLM API Key 就会继续保留。通过 Chrome Web Store 或 Microsoft Edge 加载项商店正常升级时，扩展 ID 同样保持不变，一般不需要重新填写 Key。

从 v1.2.6 起，发布包在 `manifest.json` 中包含固定的扩展身份公钥。即使开发者模式下更换了解压目录，Chrome / Edge 也可以据此生成稳定的扩展 ID。首次从更早版本切换到 v1.2.6 时扩展 ID 可能改变一次，需要重新填写一次 Key；之后请保留该 `key` 字段，不要在二次开发时重新生成。

---

## 🧠 当前工作原理

```text
打开 B 站视频
      ↓
识别当前 BV / 分 P / 合集分集
      ↓
扩展后台请求 /x/player/wbi/v2
      ↓
校验 bvid / cid → 读取官方字幕
      ↓
用户点击 AI 总结
      ↓
字幕就绪 → LLM 分析
      ↓
概览 / 中文章节 / 观点 / 笔记
      ↓
按视频保存到本地

（如果用户主动启用本地 Whisper 且视频无官方字幕：）
侧边栏触发转写 → 下载音轨 → 本机 faster-whisper → 字幕
```

当前设计坚持：

> **官方字幕优先读取，Whisper 按需启用；所有分析结果按视频隔离；音轨不出本机（除非你自己接入外部 ASR）。**

---

## ⚡ 本地 Whisper 速度说明

启用本地 Whisper 后，无字幕视频的完整流程包含：

```text
获取视频播放信息
下载 B 站音轨到本机
启动 / 复用 faster-whisper server (127.0.0.1:7860)
提交转录任务
等待 faster-whisper 完成
拉取识别结果（可选 zhconv 繁→简）
LLM 生成概览
```

因此它一定会比直接读取官方字幕慢。

视频越长、模型越大（`base` vs `large-v3`）、机器越慢，耗时越明显。`tiny` / `base` 适合快速试看，`medium` / `large-v3` 适合追求精度的长视频。

---

## ⚠️ HTTP 403 是什么？

B 站音轨地址通常带有临时鉴权参数，可能在一段时间后失效。

如果出现：

```text
语音识别失败
B站音轨下载失败：HTTP 403
```

可以依次尝试：

1. 刷新当前 B 站视频页面。
2. 确认视频可以正常播放。
3. 确认该视频在 B 站播放器里能看到字幕轨；如果连 B 站自己都没字幕，扩展也没辙。
4. 如果启用了本地 Whisper，确认 127.0.0.1:7860 端口可达、模型已下载。
5. 如果扩展刚更新，在扩展管理页面重新加载扩展。
6. 私密、付费、地区限制视频可能仍无法下载音轨或读取字幕。

扩展会为 B 站媒体请求补充必要的来源信息，但无法保证所有受限视频都可以识别。

---

## 📖 使用流程

1. 打开一个公开的 B 站视频。
2. 点击“转发”右侧的 **AI 总结**。
3. 在侧边栏确认当前视频标题和 UP 主。
4. 默认情况下，扩展会读取 B 站官方字幕（如果有）；无字幕视频且启用了本地 Whisper 时，等待转录完成。
5. 打开 **概览** 查看中文章节和关键观点。
7. 点击章节或字幕时间戳跳转视频。
8. 在 **笔记** 中记录内容。
9. 导出 Markdown、HTML，或复制到飞书。

---

## 📁 项目结构

```text
bililearn/
├── manifest.json              # Chrome / Edge 扩展配置
├── background.js              # API、ASR、缓存与后台消息
├── content.js                 # B 站按钮、播放器和视频切换
├── settings.js                # 设置读取与兼容逻辑
├── sidepanel.html             # 侧边栏页面
├── sidepanel.css              # 侧边栏 B 站主题样式
├── sidepanel.js               # 字幕、概览、笔记与导出
├── options.html               # 设置页面
├── options.css
├── options.js
├── prompts/                   # AI 分析提示词
├── rules/                     # B 站音轨网络规则
├── icons/                     # 扩展图标
├── tests/                     # 自动化测试
├── scripts/                   # 发布检查与打包脚本
├── PRIVACY.md
├── SECURITY.md
├── LICENSE
└── README.md
```

---

## 🧱 技术栈

```text
Chrome Extension Manifest V3
Vanilla JavaScript
HTML5
CSS3
Side Panel API
Declarative Net Request
LLM API（默认 minimax，可换 DeepSeek / GLM 等 OpenAI 兼容服务）
本地 Whisper（按需启用：faster-whisper + zhconv）
```

没有使用 React、Vue 或其他前端框架，扩展加载后可以直接运行。

---

## 🛠️ 本地开发

```bash
# 安装开发依赖
npm install

# 运行全部测试
npm test

# 运行发布检查
npm run check

# 生成 ZIP 安装包
npm run package
```

输出文件位于：

```text
dist/bililearn-v1.2.8.zip
```




---

## ⚠️ 当前局限

- 主要适配普通 B 站视频页，番剧、直播、课堂等页面结构可能不同。
- 私密、付费、地区受限或需要额外权限的视频可能无法读取字幕或音轨。
- 启用本地 Whisper 后需要等待音轨下载和转录；长视频 + 大模型会很慢。
- LLM 服务的免费额度、价格和可用性由对应平台决定。
- B 站修改网页结构或内部接口后，扩展可能需要更新。
- 其他 Chromium 浏览器即使版本符合，也可能缺少 Side Panel API。

---

## 🔐 隐私

当前版本：

- 没有账号系统
- 没有自建中转服务器
- 没有埋点统计
- 不收集用户的视频观看记录
- API Key 保存在浏览器本地扩展存储
- 字幕分析时会将文本发送给配置的 AI 服务
- 启用本地 Whisper 时，视频音轨会从 B 站下载到本机，由 faster-whisper 在 127.0.0.1:7860 转录，不会发到任何第三方

详细说明见 `PRIVACY.md` 和 `SECURITY.md`。

请不要提交包含真实 API Key 的 Issue、截图或代码。

---

## 🤝 上游项目与二次开发说明

本项目基于：

> [zarazhangrui/youtube-digest](https://github.com/zarazhangrui/youtube-digest)

进行二次开发。

主要改造包括：

- 从 YouTube 页面适配为哔哩哔哩视频页面
- 从 YouTube 视频识别改为 BV / 分 P / 合集识别
- 重新实现 B 站操作栏按钮和播放器交互
- 使用 B 站主题的中文侧边栏和设置页
- 原生字幕接口改为 `/x/player/wbi/v2`，并校验返回的 `bvid` 与 `cid`
- 增加视频状态隔离，避免切换视频后内容错乱
- 之前的云端 ASR 方案已下线；当前 ASR 由本机 faster-whisper（按需启用）承担
- 增加中文章节、全屏笔记和多格式导出
- 移除对 YouTube 和 Supadata 的运行依赖

感谢原作者公开项目和核心产品思路。

---

## 📄 License

本项目使用 [MIT License](LICENSE)。

你可以自由使用、修改、Fork、二次开发、部署和商业使用，但需要保留 MIT License 要求的版权和许可声明，并尊重上游项目的原始许可证。

---

## 💬 项目理念

bililearn 不想只做一个“把字幕复制出来”的工具。

它更希望把长视频变成真正可以继续使用的资料：

```text
能读
能搜
能跳转
能总结
能记笔记
能导出
```

默认从 B 站官方字幕读起，用户主动启用本地 Whisper 时才转录音轨；所有分析按视频隔离，音轨不出本机。

> **不自动消耗额度，不混淆不同视频，也不让中文用户拿到一套莫名其妙的英文章节。**

---

## ⭐ 如果这个项目对你有帮助

欢迎 Star、Fork、提交 Issue 或 Pull Request。

遇到问题时，请提供浏览器版本、扩展版本、B 站视频链接、是否有官方字幕、错误提示及可公开的控制台日志，并先删除 API Key、Cookie 和个人账号信息。
