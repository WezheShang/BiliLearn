...
3. 开启右上角“开发者模式”。
4. 点击“加载已解压的扩展程序”。
5. 选择包含 `manifest.json` 的 `bililearn` 文件夹。
...
minimax API Key、笔记和缓存只保存在当前 Chrome 配置中。字幕从 B 站接口读取；使用 AI 功能时，相关字幕和视频信息会发送给 minimax。不要把 API Key 写进源码、提交记录、截图或聊天。
...
打包结果位于 `dist/bililearn-v1.2.2.zip`。
